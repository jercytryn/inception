function [bestClassification, info] = ClassifyLinesWithKnownFocal(lines,  ParaList, linesInImage)
%%This function is to classify lines according to their directions in the Manhattan world.
%Input:  
%        lines        --  the normalized lines, which lies in the normalized image plane with unit focal length.
%        ParaList.kMaxNumHyp
...
%                     --  the way to refine the estimated values and classification results: 'Iter', 'SVD', or 'AlgLS'.
%                         Note that, SVD and AlgLS are defined only when the consistency measure is CM1 (i.e. kConsistencyMeasure=1). 
%        ParaList.kMat--  the camera intrinsic parameters. If kConsistencyMeasure=1, then this parameter is needed.
%        linesInImage --  the extracted lines from image, [i, startPointx, startPointy, endPointx, endPointy]
%                         If kConsistencyMeasure=1, then this parameter is needed.
...
if kConsistencyMeasure == 2 && strcmp(kRefineMethod, 'Iter')
...
hypoth2A = ceil(numLinesInpeak2 * rand(kMinNumHyp,1));%only kMinNumHyp hypotheses for situation A.
...
for hypCount = 1:kMinNumHyp
    ...
    for resulutsID = 1: numofResults
        ...
        if kConsistencyMeasure==1 %'CM1'
            ...
            for lineID = 1:numOfLines
                mp          = middlePoints(lineID,:);%homogenous coordinate
                startPoint  = [linesInImage(lineID,2:3), 1];
...
if bestNumInliers/numOfLines < kAcceptHypothesisThreshold
   ...
   hypoth2B = ceil((numOfLines-numLinesInpeak1) * rand(kMaxNumHyp,1)); %at most kMaxNumHyp hypotheses for situation B.
   log01 = log(0.01);
...
elseif strcmp(kRefineMethod, 'Iter')
    ...
    if kConsistencyMeasure==1
        [refinedBestVPXYZ, residual] = RefineVPByNewtonIteration(linePlaneNormal(bestClassification ~= 0,:), ...
            bestVPXYZ, bestClassification(bestClassification ~= 0), kConsistencyMeasure);
    else
        [refinedBestVPXYZ, residual] = RefineVPByNewtonIteration(linePlaneNormal(bestClassification ~= 0,:), ...
            bestVPXYZ, bestClassification(bestClassification ~= 0),...
            kConsistencyMeasure,...
            linesInImage(bestClassification ~= 0,:), kMat);
    end
else
    if strcmp(kRefineMethod, 'AlgLS')
    [refinedBestVPXYZ] = RefineVPByAlgLS(linePlaneNormal, bestClassification);
    else
        error('Please choose the correct way to refine: SVD, Iter or AlgLS');
    end
end
timeToRefine = toc(timeStartRefine);
%re-classify.
newClassification = zeros(1,numOfLines);
if kConsistencyMeasure == 1 || ~strcmp(kRefineMethod, 'Iter')
    for lineID = 1:numOfLines
